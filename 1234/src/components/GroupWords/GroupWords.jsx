...
const GroupWords = ({ history, groupId, name }) => {
  ...
  const DifficultWordsButton = () => (
    <span className="d-inline-block mr-auto ml-auto">
      <S.NotebookButton
        ...
        onClick={() =>
          handleClick(`/rslang/textbook/group/${groupId}/difficult_words`)
        }
      >
        Сложные
      </S.NotebookButton>
  ...
  const DeletedWordsButton = () => (
    <span className="d-inline-block mr-auto ml-auto">
      <S.NotebookButton
        disabledbutton={currentUser ? null : 'true'}
        variant="outline-primary"
        onClick={() => handleClick(`/rslang/textbook/group/${groupId}/deleted_words`)}
      >
        Удаленные
      </S.NotebookButton>
    </span>
  ...
  return (
    <S.GroupWords isUserLoggedIn={!!currentUser}>
      ...
      <S.NotebookButton
        variant="outline-primary"
        onClick={() => history.push(`/rslang/textbook/group/${groupId}`)}
      >
        Общее
      </S.NotebookButton>
      {currentUser ? (
